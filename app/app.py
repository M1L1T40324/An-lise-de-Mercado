...
st.title("📊 Downloader de Dados Financeiros + Dashboard com Regressão e Indicadores")

# Input tickers
tickers_input = st.text_input("Digite o(s) ticker(s) separados por vírgula (ex: PETR4.SA, AAPL, BTC-USD):")
tickers = [t.strip().upper() for t in tickers_input.split(",") if t.strip()]

# Input período (opcional)
periodo = st.selectbox("Período para análise", ["1y", "2y", "5y", "max"], index=3)

if st.button("Baixar dados"):
    if tickers:
        st.write(f"Baixando dados de **{', '.join(tickers)}**...")
        df = yf.download(tickers, group_by='ticker', auto_adjust=True, period=periodo)
        df = df.dropna(how="all")  # apenas linhas totalmente vazias
        ...
        if df.empty:
            ...
            for ticker in tickers:
                ...
                ticker_df = ticker_df.dropna(subset=['Close'])  # garante que Close exista
                ...
                if ticker_df["Close"].dropna().empty:
                    ...
                    z_score = float((ticker_df["Close"].iloc[-1] - ticker_df["Close"].mean()) / ticker_df["Close"].std())
                ...
                # Gráfico Preço + Regressão Linear
                fig_reg = go.Figure()
                fig_reg.add_trace(go.Scatter(
                    x=ticker_df['Date'], y=ticker_df['Close'], mode='lines', name='Close Real'
                ))
                fig_reg.add_trace(go.Scatter(
                    x=ticker_df['Date'], y=ticker_df['Close_Pred'], mode='lines',
                    name='Regressão Linear', line=dict(dash='dash', color='orange')
                ))
                ...
                # Gráfico Variação do Desvio
                fig_dev = go.Figure()
                fig_dev.add_trace(go.Scatter(
                    x=ticker_df['Date'], y=ticker_df['Deviation_Change'], mode='lines', name='Variação do Desvio'
                ))
                ...
                # Gráfico Volume (aparece só para período ≤ 1 ano)
                if periodo in ['1y']:
                    fig_vol = go.Figure()
                    fig_vol.add_trace(go.Bar(
                        x=ticker_df['Date'], y=ticker_df['Volume'], name='Volume'
                    ))
                    fig_vol.update_layout(
                        title=f"{ticker} - Volume de Negociações",
                        plot_bgcolor='rgb(20,20,20)',
                        paper_bgcolor='rgb(20,20,20)',
                        font=dict(color='white')
                    )
